...
use super::store::GetterType;
use super::store::IncrementerType;
...
fn unwrap_noreply(tag: Option<&[u8]>) -> bool {
    match tag {
        ...
        Some(_) => panic!("can't unwrap noreply tag {:?}", tag),
        None => true,
    }
}
...
fn map_setter_name(res: &[u8]) -> SetterType {
    match res {
        ...
        _ => panic!("unknown setter mapped? {:?}", res),
    }
}
...
fn map_getter_name(res: &[u8]) -> GetterType {
    match res {
        ...
        _ => panic!("unknown getter mapped? {:?}", res),
    }
}
...
pub fn parse_size(size_str: &str) -> Option<usize> {
    ...
    match re.captures(size_str) {
        None => None,
        Some(matches) => {
            let digits = matches.get(1).unwrap().as_str();
            let number: usize = FromStr::from_str(digits).unwrap();
            let suffix = matches.get(2).map(|m| m.as_str());
            let mult = match suffix {
                ...
                bad_mult => {
                    unreachable!("weird suffix {:?}", bad_mult)
                }
            };
            Some(number * mult)
        }
